...
const resetMagnetic = (element: HTMLElement) => {
  animate(element, { x: 0, y: 0 }, { type: "spring", stiffness: 300, damping: 22 });
};

const initMagnetic = () => {
  document.querySelectorAll<HTMLElement>("[data-motion-magnetic]").forEach((element) => {
...
const initTilt = () => {
  document.querySelectorAll<HTMLElement>("[data-motion-tilt]").forEach((element) => {
    element.addEventListener("pointermove", (event) => {
      const bounds = element.getBoundingClientRect();
    ...
    element.addEventListener("pointerleave", () => {
      animate(element, { rotateX: 0, rotateY: 0 }, { type: "spring", stiffness: 220, damping: 24 });
    });
  });
};

const initSpotlights = () => {
  document.querySelectorAll<HTMLElement>("[data-motion-spotlight]").forEach((element) => {
    element.addEventListener("pointermove", (event) => {
      const bounds = element.getBoundingClientRect();
...
const initReveals = () => {
  ...
  document.querySelectorAll<HTMLElement>("[data-motion-header]").forEach((header) => {
  ...
  document.querySelectorAll<HTMLElement>("[data-motion-list]").forEach((list) => {
    const items = list.querySelectorAll<HTMLElement>(":scope > [data-motion-item]");
    ...
    inView(
      ...
      () => {
        ...
        animated.add(list);
        animateIn(Array.from(items), { y: [28, 0] }, { delay: stagger(0.08) });
      },
      { amount: 0.14 },
    );
  });

  document.querySelectorAll<HTMLElement>("main h1, main h2, main h3, main p, main .section-kicker").forEach((element) => {
    inView(
      element,
      () => {
        if (animated.has(element)) return;
